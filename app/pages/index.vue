...
const { title, github, twitter, telegram } = useAppConfig()
</script>

<template>
  <div class="flex min-h-screen flex-col bg-black dark">
      ...
      <div class="mx-auto max-w-6xl px-6">
        <div class="flex flex-col items-center gap-6 pt-2 md:flex-row md:justify-between">
          <div class="flex flex-col items-center gap-4 md:flex-row md:gap-6">
            <NuxtLink to="/" :title="title" class="flex items-center space-x-2">
              <span class="flex size-8 items-center justify-center rounded-full">
                <img src="/sink.png" :alt="title" class="size-full rounded-full" />
              </span>
              <span class="text-xl font-black text-white">{{ title }}</span>
            </NuxtLink>
            <small class="text-sm text-gray-400">
              &copy; {{ new Date().getFullYear() }}
              <a href="https://html.zone" target="_blank" class="hover:text-white">
                Produkte von HTML.ZONE
              </a>
            </small>
          </div>
